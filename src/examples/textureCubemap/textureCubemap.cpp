    ...
    class VulkanExample : public VulkanBase {
        ...
        private:
            void initVulkan() override {
                ...
                m_window.setTitle("Texture Cubemap");
            ...
            void createPipelines() override {
                ...
                VulkanShaderModule vertShaderModuleSkybox = 
                    VulkanShaderModule(
                            "src/shaders/textureCubemap/skyboxVert.spv", 
                            &m_device, 
                            VK_SHADER_STAGE_VERTEX_BIT);
                VulkanShaderModule fragShaderModuleSkybox = 
                    VulkanShaderModule(
                            "src/shaders/textureCubemap/skyboxFrag.spv", 
                            &m_device, 
                            VK_SHADER_STAGE_FRAGMENT_BIT);
            ...
            void createDescriptorSets() override {
                ...
                textureDescriptor.imageView = m_cubeMapTexture.view;
                textureDescriptor.imageLayout = m_cubeMapTexture.imageLayout;
                textureDescriptor.sampler = m_cubeMapTexture.sampler;
            ...
            void loadAssets() {
                ...
                loadCubemapTexture("src/textures/cubemap_yokohama_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, true);
            }

            void loadCubemapTexture(std::string filename, VkFormat format, bool forceLinearTiling) {
                ktxResult result;
                ...
                m_cubeMapTexture.device = &m_device;
                m_cubeMapTexture.width = ktxTexture->baseWidth;
                m_cubeMapTexture.height = ktxTexture->baseHeight;
                m_cubeMapTexture.mipLevels = ktxTexture->numLevels;
                ...
                imageCreateInfo.mipLevels = m_cubeMapTexture.mipLevels;
                imageCreateInfo.samples = VK_SAMPLE_COUNT_1_BIT;
                imageCreateInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
                imageCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
                imageCreateInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
                imageCreateInfo.extent = { m_cubeMapTexture.width, m_cubeMapTexture.height, 1};
                imageCreateInfo.usage = VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
                imageCreateInfo.arrayLayers = 6;
                imageCreateInfo.flags = VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT;

                VK_CHECK_RESULT(vkCreateImage(m_device.getLogicalDevice(), &imageCreateInfo, nullptr, &m_cubeMapTexture.image));

                VkMemoryRequirements memReqs;
                vkGetImageMemoryRequirements(m_device.getLogicalDevice(), m_cubeMapTexture.image, &memReqs);
                ...
                VK_CHECK_RESULT(vkAllocateMemory(m_device.getLogicalDevice(), &memAllocInfo, nullptr, &m_cubeMapTexture.deviceMemory));
                VK_CHECK_RESULT(vkBindImageMemory(m_device.getLogicalDevice(), m_cubeMapTexture.image, m_cubeMapTexture.deviceMemory, 0));
                ...
                for (uint32_t face = 0; face < 6; face++) {
                    for (uint32_t level = 0; level < m_cubeMapTexture.mipLevels; level++) {
                        ktx_size_t offset;
                ...
                subResourceRange.levelCount = m_cubeMapTexture.mipLevels;
                subResourceRange.layerCount = 6;

                tools::setImageLayout(
                        copyCmd.getCommandBuffer(), 
                        m_cubeMapTexture.image, 
                ...
                m_cubeMapTexture.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

                tools::setImageLayout(
                        copyCmd.getCommandBuffer(), 
                        m_cubeMapTexture.image, 
                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,  
                        m_cubeMapTexture.imageLayout, 
                        subResourceRange);
                ...
                sampler.minLod = 0.0f;
                sampler.maxLod = m_cubeMapTexture.mipLevels;
                ...
                VK_CHECK_RESULT(vkCreateSampler(m_device.getLogicalDevice(), &sampler, nullptr, &m_cubeMapTexture.sampler));
                ...
                imageView.subresourceRange.levelCount = m_cubeMapTexture.mipLevels;
                imageView.image = m_cubeMapTexture.image;
                VK_CHECK_RESULT(vkCreateImageView(m_device.getLogicalDevice(), &imageView, nullptr, &m_cubeMapTexture.view));
            ...
            void OnUpdateUI (UI *ui) override {
                if (ui->header("Settings")) {
                    if (ui->sliderFloat("LOD bias", &m_uboVS.lodBias, 0.0f, (float)m_cubeMapTexture.mipLevels)) {
                        updateUniformBuffers();
                    }
